...
export default function WalkthroughPage() {
  ...
  const updateRoute = useCallback(
    (newStage: WalkthroughStage, newStep: number) => {
      const params = new URLSearchParams(searchParams.toString());
      params.set("tourStage", newStage);
      params.set("step", newStep.toString());
      router.replace(`/walkthrough?${params.toString()}`, { scroll: false });
    },
    [router, searchParams],
  );
  ...
  const nextStep = () => {
    if (currentStep < currentSteps.length - 1) {
      updateRoute(stage, currentStep + 1);
    } else if (stage === "discovery") {
      updateRoute("creator", 0);
    } else {
  ...
  const prevStep = () => {
    if (currentStep > 0) {
      ...
      const prevDiscoverySteps = isMobile ? mobileDiscoverySteps : desktopDiscoverySteps;
      updateRoute("discovery", prevDiscoverySteps.length - 1);
    }
  };
  ...
  return (
    <ThemePage className={cn("max-w-[1600px] mx-auto py-0 md:py-16 px-2 md:px-12 min-h-screen lg:min-h-0", "md:pt-16")}>
          ...
          >
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                >
                  {stage === "discovery" ? "Global Discovery" : "Creation Studio"}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-start gap-1.5 font-sans text-[9px] text-foreground-subtle">
              <div className="flex items-center gap-2.5">REV.16 / {stage.toUpperCase()}</div>
              <div>
      ...
      >
        ...
        <div
          className={cn(
            "lg:col-span-6 space-y-4 md:space-y-6 lg:space-y-8 lg:sticky lg:top-32 z-[410] pointer-events-auto",
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                ...
                <div className="space-y-4 md:space-y-6 mb-4 md:mb-2 max-h-[30vh] md:max-h-[40vh] lg:max-h-none overflow-y-auto pr-2 custom-scrollbar hidden md:block">
                ...
                <div className="flex gap-4 pt-4 md:pt-4 border-t border-noir-border">
                  <Button
                    variant="ghost"
                    onClick={prevStep}
                    disabled={stage === "discovery" && currentStep === 0}
                    className="border border-noir-border flex-1 h-10 md:h-12 uppercase tracking-widest group rounded-full font-sans text-[10px] hover:bg-noir-hover disabled:opacity-20 disabled:pointer-events-none"
                  >
                  ...
                  <Button
                    onClick={nextStep}
                    className="flex-1 h-10 md:h-12 transition-all uppercase tracking-widest gap-2 group rounded-full bg-white text-black hover:bg-gray-200 font-sans text-[10px] font-bold"
                  >
        ...
        <div
          className={cn(
            "lg:col-span-6 lg:mt-0 z-[450]",
            "fixed left-0 right-0 px-2 md:px-0 md:relative md:bottom-auto md:left-auto md:right-auto",
            stage === "creator" ? "bottom-4" : "bottom-24",
          )}
        >
          <div className="relative aspect-[16/11] md:aspect-[4/3] bg-black border border-noir-border overflow-hidden">
            <div
            ...
            >
              ...
              >
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            <Icon
                              size={48}
                              className="text-accent/80 drop-shadow-[0_0_20px_rgba(var(--accent-rgb),0.3)] md:hidden"
                            />
                            <Icon
                              size={80}
                              className="text-accent/80 drop-shadow-[0_0_20px_rgba(var(--accent-rgb),0.3)] hidden md:block"
                            />
                          </>
                        );
                      })()}
                ...
                <div className="mt-4 md:mt-8 text-center animate-pulse">
                  <div className="text-[8px] md:text-[10px] font-mono text-foreground-subtle uppercase tracking-[0.4em] mb-2">
          ...
          >
            ...
            <div className="text-xs tracking-tight leading-relaxed hidden sm:block font-serif italic text-foreground-muted">
              The system is currently highlighting the {currentStepData.location} module within your workspace for
              visual context.
            </div>
          </motion.div>
        </div>
