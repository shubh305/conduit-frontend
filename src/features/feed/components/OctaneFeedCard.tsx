...
export function OctaneFeedCard({ item, className, onRemove }: OctaneFeedCardProps) {
  ...
  return (
    <div className={cn(
      "group relative flex flex-col h-[400px] bg-[#111] overflow-hidden border border-[#333] hover:border-[#E65100] transition-colors duration-300 shadow-xl",
      className
    )}>
      {/* 1. Carbon Fiber Accent */}
      <div className="absolute top-0 right-0 w-24 h-full bg-[#151515] skew-x-[-15deg] translate-x-12 z-0 border-l border-[#222]" 
           style={{ 
             backgroundImage: `linear-gradient(45deg, #181818 25%, transparent 25%, transparent 75%, #181818 75%, #181818), linear-gradient(45deg, #181818 25%, transparent 25%, transparent 75%, #181818 75%, #181818)`, 
      ...
      <Link href={postUrl} className="relative h-[55%] w-full overflow-hidden block group/image">
         {item.featuredImage ? (
            ...
            <div className="absolute inset-0 bg-[#222] flex items-center justify-center">
               <span className="text-[#333] font-black italic text-4xl uppercase tracking-tighter skew-x-[-10deg]">NO_SIGNAL</span>
            </div>
         )}
         
         {/* Speedometer Overlay */}
         <div className="absolute bottom-0 left-0 w-full h-1/2 bg-gradient-to-t from-[#111] to-transparent pointer-events-none" />
         
         {/* Badge */}
         <div className="absolute top-4 left-4 bg-[#E65100] text-white font-black text-xs px-3 py-1 skew-x-[-10deg] uppercase shadow-[4px_4px_0_rgba(0,0,0,0.5)] transform translate-x-[-20px] opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
      ...
      <div className="relative z-10 flex flex-col p-6 h-[45%] justify-between">
          <div className="flex flex-col gap-2">
             ...
             <Link href={postUrl} className="group/title">
                <h3 className="text-2xl font-black italic text-white uppercase leading-[0.9] tracking-tighter transition-colors group-hover/title:text-[#E65100] line-clamp-3 font-sans">
                   {item.title}
                </h3>
             </Link>
          </div>
          ...
          <div className="mt-auto pt-4 border-t border-[#222] flex items-center justify-between">
             <div className="flex gap-2">
                {item.tags.slice(0, 2).map((tag) => (
                   <span key={tag} className="text-[9px] bg-[#1A1A1A] text-[#888] px-2 py-1 uppercase font-bold tracking-wider hover:bg-[#E65100] hover:text-white transition-colors cursor-pointer skew-x-[-10deg]">
                      #{tag}
                   </span>
                ))}
             </div>
             
             <div className="flex items-center gap-3 text-[#555] hover:text-[#E65100] transition-colors">
                 <PostActions 
                   data={item} 
                   onRemove={onRemove}
