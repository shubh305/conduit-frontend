      ...
      <main className="max-w-screen-xl mx-auto w-full px-6 md:px-12 py-8 md:py-12 flex flex-col gap-10">
        ...
        {featured && (
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center group cursor-pointer">
            ...
            <div className="lg:col-span-4 flex flex-col justify-center gap-4">
               ...
               <p className="text-foreground-muted leading-relaxed text-lg line-clamp-3">
                  {featured.excerpt}
               </p>
        ...
        {items.length > 0 ? (
          <section className="flex flex-col">
             {listItems.map((item) => (
               <article key={item.postId} className="mb-5 p-5 group grid grid-cols-1 md:grid-cols-[1fr_240px] lg:grid-cols-[1fr_280px] xl:grid-cols-[160px_1fr_240px] gap-8 py-8 border-t border-border-primary first:border-none items-start">
                  ...
                  <div className="flex flex-col gap-3">
                     ...
                     <div className="flex xl:hidden items-center gap-3 text-xs mb-2">
                        ...
                        <span className="text-foreground-subtle">{new Date(item.publishedAt).toLocaleDateString()}</span>
                     </div>
                     ...
                     <p className="text-foreground-muted leading-relaxed line-clamp-2 md:line-clamp-3">
                        {item.excerpt}
                     </p>
                     
                     <div className="mt-2 flex items-center gap-4">
                        <Link href={getPostUrl(item)} className="text-sm font-bold text-foreground flex items-center gap-1 group-hover:gap-2 transition-all">
                  ...
                  <div className="hidden md:block pl-4 xl:pl-0">
                     <Link href={getPostUrl(item)} className="block relative aspect-[3/2] w-full max-w-[280px] ml-auto rounded-sm overflow-hidden bg-bg-panel border border-border-primary shadow-sm hover:shadow-md transition-all">
                        {item.featuredImage && (
                          <Image
