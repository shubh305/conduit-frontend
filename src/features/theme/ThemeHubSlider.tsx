  ...
  return (
    ...
    >
          ...
          {THEMES.map((t) => (
            <div 
              ...
              className={cn(
                ...
                theme === t ? "bg-accent scale-[2.5] blur-[0.5px] shadow-[0_0_10px_var(--accent)]" : 
      ...
      <div className="flex justify-between px-2">
        <button 
          onClick={() => {
            const prev = (currentIndex - 1 + totalThemes) % totalThemes;
            setTheme(THEMES[prev]);
          }}
          style={{ borderRadius: baseRadius > 0 ? "4px" : "0px" }}
          className={cn(
            "text-[9px] uppercase tracking-widest transition-all py-1.5 px-3 border hover:shadow-sm",
            isDarkMode ? "text-white/40 border-white/10 hover:bg-white/5 hover:text-white" : "text-black/40 border-black/10 hover:bg-black/5 hover:text-black",
          )}
        >
        ...
        <button 
          onClick={() => {
            const next = (currentIndex + 1) % totalThemes;
