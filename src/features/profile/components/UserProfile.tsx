...
export function UserProfile() {
  ...
  // Detect current tenant subdomain for correct link generation
  const [currentTenantSlug, setCurrentTenantSlug] = useState<string | undefined>(undefined);
  useEffect(() => {
    if (typeof window !== "undefined") {
      const hostname = window.location.hostname;
      const parts = hostname.split(".");
      // Check for subdomain (e.g. tenant.domain.com or tenant.localhost)
      // We assume standard 2-level TLD or localhost
      if (parts.length > 2 || (hostname.includes("localhost") && parts.length >= 2)) {
        const subdomain = parts[0];
        if (subdomain !== "www" && subdomain !== "localhost") {
          setCurrentTenantSlug(subdomain);
        }
      }
    }
  }, []);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-foreground-subtle font-mono">
        {loadingText}
      </div>
    );
  }
  ...
  if (error || !profile) {
    return (
      <div className="min-h-screen flex items-center justify-center text-signal-red font-mono">
        {error || "User not found"}
      </div>
    );
  }

  const uptimeDays = profile.joinedAt
    ? Math.floor((new Date().getTime() - new Date(profile.joinedAt).getTime()) / (1000 * 60 * 60 * 24))
    : 0;
  const joinDate =
    profile.joinedAt || profile.createdAt
      ? new Date(profile.joinedAt || profile.createdAt || "").toLocaleDateString("en-US", {
          month: "short",
          year: "numeric",
        })
      : "OCT, 2018";
  ...
  return (
    ...
    >
      ...
      <section className="space-y-8">
        ...
        {isTerminalCopy ? (
          <TerminalActivityLog posts={posts} currentTenantSlug={currentTenantSlug} />
        ) : (
              ...
              <div key={post.postId}>
                <div className="relative pl-4 md:pl-12">
                  ...
                  <div className="space-y-4">
                    ...
                    <FeedCard item={post} variant="compact" index={idx} currentTenantSlug={currentTenantSlug} />
                  </div>
                </div>
              </div>
            ))}
...
function TerminalActivityLog({ posts, currentTenantSlug }: { posts: FeedItem[]; currentTenantSlug?: string }) {
  return (
    <div className="font-mono text-xs md:text-sm">
          ...
          {posts.map(post => (
            <div key={post.postId} className="group flex items-center hover:bg-accent/5 -mx-2 px-2 transition-colors">
              ...
              <div className="flex-1">
                <TerminalFeedCard item={post} currentTenantSlug={currentTenantSlug} />
              </div>
            </div>
          ))}
        </div>
