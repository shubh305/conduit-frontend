...
export function CyberArticleLayout({ post, tenant, isPreview: isPreviewProp }: ArticleLayoutProps) {
  ...
  return (
    <main className={cn("min-h-screen text-foreground transition-all duration-700", isRoninCopy || isSakuraCopy ? "bg-transparent" : "bg-noir-bg")}>
      {/* Immersive Header */}
      <div
        className={cn(
          "border-b border-noir-border min-h-[500px] flex flex-col relative justify-end pb-20 px-6 md:px-24 transition-all",
          isSakuraCopy && "bg-gradient-to-br from-noir-bg via-noir-bg to-accent/10",
        )}
      >
        {/* Background Decor */}
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          <div className={cn("absolute top-20 right-[10%] w-[500px] h-[500px] blur-[150px] rounded-full transition-opacity opacity-20", isCyberCopy ? "bg-accent" : "bg-accent/40")} />
          {isCyberCopy && (
            <div
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full">
          {!isPreview && (
            <Link
              ...
              className={cn(
                ...
                isCyberCopy ? "font-mono text-[10px] text-accent uppercase tracking-[0.3em]" : "text-sm text-foreground-subtle",
              )}
            >
              <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
              {isSakuraCopy ? "フィードに戻る" : isRoninCopy ? "Return (帰還)" : isCyberCopy ? "BACK_TO_FEED" : "Back to Blog"}
            </Link>
          )}
            ...
            {post.tags.map((tag: string) => (
              <span
                ...
                className={cn(
                  ...
                  isCyberCopy ? "font-mono text-accent border-accent/20 bg-accent/5" : "font-sans text-foreground-subtle border-noir-border bg-noir-panel",
                )}
              >
                #{tag}
              </span>
          ...
          <h1
            className={cn(
              "text-5xl md:text-8xl font-black text-foreground mb-12 leading-[0.9] tracking-tighter break-words max-w-5xl transition-all",
              isCyberCopy ? "font-display italic uppercase" : config.fontFamily === "serif" ? "font-serif italic" : "font-sans",
            )}
          >
            {post.title}
          </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-noir-border pt-10 mt-10 w-full">
            <div className="flex flex-col gap-2">
              <span className="text-[9px] font-mono text-foreground-subtle uppercase tracking-[0.2em]">{isSakuraCopy ? "著者" : isRoninCopy ? "Author (作者)" : "Author"}</span>
              <div className="flex items-center gap-3">
                <div className={cn("w-7 h-7 flex items-center justify-center text-[10px] font-bold border border-noir-border bg-noir-panel shadow-sm", isCyberCopy ? "rounded-none" : "rounded-full")}>
                  {post.authorName.charAt(0)}
                </div>
                <span className={cn("text-foreground font-bold text-xs uppercase tracking-widest", isCyberCopy ? "font-mono" : "font-sans")}>{post.authorName}</span>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <span className="text-[9px] font-mono text-foreground-subtle uppercase tracking-[0.2em]">{isSakuraCopy ? "公開日" : isRoninCopy ? "Date (日付)" : "Published"}</span>
            ...
            <div className="flex flex-col gap-2">
              <span className="text-[9px] font-mono text-foreground-subtle uppercase tracking-[0.2em]">{isSakuraCopy ? "読了時間" : isRoninCopy ? "Time (時間)" : "Read Time"}</span>
      ...
      <div className="max-w-[1920px] mx-auto px-6 md:px-0">
        <article className="max-w-4xl mx-auto py-20 md:py-32">
          {post.featuredImage && (
            <div
              className={cn("w-full aspect-video border border-noir-border mb-20 relative overflow-hidden shadow-2xl transition-all duration-1000 group", isCyberCopy ? "rounded-none" : "rounded-3xl")}
            >
          ...
          <div
            className={cn(
              ...
              isCyberCopy ? "prose-headings:font-display prose-headings:uppercase prose-p:font-mono prose-a:text-accent" : "prose-a:text-accent prose-headings:font-sans",
