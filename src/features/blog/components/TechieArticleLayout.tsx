      ...
      <div className="border-b border-white/10 bg-noir-bg">
        <div className="container mx-auto max-w-[1600px]">
           <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[500px]">
              ...
              {/* Header Left: Content */}
              <div className="p-8 lg:p-16 flex flex-col justify-center gap-8 relative overflow-hidden">
                 ...
                 <div className="inline-flex items-center gap-2 px-3 py-1 bg-noir-panel/50 rounded-sm border border-white/10 w-fit backdrop-blur-sm shadow-sm">
                    ...
                    <span className="text-[10px] font-mono text-accent uppercase tracking-widest">Transmission Recieved</span>
                 </div>

                 <h1 className="text-4xl md:text-5xl lg:text-7xl font-sans font-black leading-[1.1] tracking-tight text-white uppercase shadow-lg">
                   {post.title}
                 </h1>

                 {/* Author & Meta Block */}
                 <div className="flex flex-wrap items-center gap-8 border-t border-white/5 pt-8 mt-auto">
                    <div className="flex items-center gap-4 group cursor-pointer">
                        ...
                        <div>
                           ...
                           <div className="text-sm font-sans font-bold text-white group-hover:text-accent/80 transition-colors uppercase">{post.authorName}</div>
                        </div>
                    </div>
              ...
              <div className="relative h-[300px] lg:h-auto border-l lg:border-l border-white/5 overflow-hidden group shadow-[inset_0_0_100px_rgba(0,0,0,0.5)]">
                  ...
                  {post.featuredImage ? (
                     <Image
                       ...
                       alt={post.title}
                       fill
                       className={cn("object-cover transition-all duration-1000 scale-100 group-hover:scale-105 opacity-80 group-hover:opacity-100", isDarkMode && "grayscale-[20%] group-hover:grayscale-0")}
                     />
                  ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] relative">
           ...
           <aside className="hidden lg:block border-r border-white/5 bg-noir-bg">
              <div className="sticky top-20 p-8 space-y-12">
                 ...
                 {/* Back Nav */}
                 {!isPreview && (
                   <Link href={`/${tenant.slug || tenant.id}`} className="group flex items-center gap-3 text-xs font-mono text-accent hover:text-white transition-colors">
                      <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" />
                      <div>
                         <span className="block font-black tracking-tighter italic uppercase text-sm">INDEX.SYS</span>
                         <span className="block text-[8px] text-accent/40 tracking-[0.2em] mt-0.5 opacity-50 uppercase">{tenant.name}</span>
                      </div>
                   </Link>
                 )}
                    ...
                    <div className="border-l-2 border-accent/40 pl-4">
                       ...
                       <div className="flex flex-col gap-2.5">
                         {post.tags.map((tag: string) => (
                           <span key={tag} className="font-mono text-[11px] text-foreground/40 hover:text-accent transition-colors block truncate hover:translate-x-1 duration-200">
                             #{tag}
                           </span>
                         ))}
                       </div>
                 ...
                 <div className="border-l-2 border-white/5 pl-4 hover:border-accent/40 transition-colors">
                    ...
                    <div className="space-y-3 text-[10px] font-mono text-foreground/50">
                       ...
                       <div className="flex justify-between items-center border-b border-white/5 pb-1">
                         ...
                         <span className="font-bold text-white/80">{post.readingTimeMinutes ? Math.round(post.readingTimeMinutes * 230) : 0}</span>
                       </div>
                    </div>
                 </div>
           ...
           <div className="p-8 md:p-12 lg:p-20 relative">
               ...
               <div 
                 ...
                 className="prose prose-invert prose-lg max-w-[800px] mx-auto
               ...
               <FeedActionBar
                 ...
                 className="mt-20 border-t border-white/5 py-12 max-w-[800px] mx-auto opacity-80"
