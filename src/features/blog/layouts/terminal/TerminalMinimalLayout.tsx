 ...
 */
export function TerminalMinimalLayout({ posts, tenantSlug, currentTenantSlug }: LayoutProps) {
  return (
    <div className="w-full h-full font-mono flex flex-col">
      ...
      <div className="border border-accent/30 bg-black flex-1 overflow-y-auto custom-scrollbar shadow-[0_0_15px_rgba(34,197,94,0.05)]">
        {posts.length === 0 ? (
          <div className="py-12 text-center text-accent/30 italic text-sm">[EMPTY]</div>
        ) : (
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h3
                          className={cn(
                            "text-foreground transition-colors font-bold uppercase truncate mb-1",
                            TERMINAL_HOVER_TEXT,
                          )}
                        >
                          {post.title}
                      ...
                      <div className={cn("text-[10px] shrink-0 text-right", TERMINAL_HOVER_TEXT_MUTED)}>
                        {post.likesCount !== undefined && <div>♥ {post.likesCount}</div>}
                        {post.commentsCount !== undefined && <div>◯ {post.commentsCount}</div>}
                      </div>
                    </div>
                  </div>
                </Link>
