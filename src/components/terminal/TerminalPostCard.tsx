...
export function TerminalPostCard({
  ...
  const size = post.readingTimeMinutes
    ? `${post.readingTimeMinutes * 1024}`
    : "4096";
  ...
  if (variant === "minimal") {
    return (
      <Link
        href={postLink}
        className={cn(
          "block font-mono text-sm py-1 px-2 transition-colors group",
          TERMINAL_HOVER_CONTAINER,
          className
        )}
      >
        <span className={cn("font-bold", TERMINAL_HOVER_TEXT)}>
          {post.title}
        </span>
      </Link>
    );
  ...
  if (variant === "compact") {
    return (
      ...
      >
        ...
        <span className={cn("font-bold truncate", TERMINAL_HOVER_TEXT)}>
          {post.title}
        </span>
      </Link>
    );
  ...
  return (
    <div className={cn("group", className)}>
      ...
      {/* Actions slot */}
      {actions && (
        <div className="pl-4 mt-1">{actions}</div>
      )}
    </div>
  );
}
